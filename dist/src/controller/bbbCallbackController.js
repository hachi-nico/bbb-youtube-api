...
const listenRecordingReady = async (req, res) => {
  ...
  if (signed_parameters) {
    ...
    if (!manualFilename) {
      ...
      return res.status(500).json(resError("Gagal saat melakukan action callback"));
    }
  }
  bbbCallbackBody = {
  ...
  if (!secret) {
    await createLaporan(manualTitle ?? titleFormat, manualDescription ?? bbbCallbackBody.record_id, dayjs().format(insertDateTimeFormat), "", 5, 0);
  ...
  const titleFormat = `${Math.floor(Math.random() * 1001)} ${bbbCallbackBody.meeting_id} ${dayjs().format(insertDateTimeFormat)}`;
  const insertLaporan = await createLaporan(manualTitle ?? titleFormat, manualDescription ?? bbbCallbackBody.record_id, dayjs().format(insertDateTimeFormat), "", isUploading ? 4 : 2, 0);
  if (!insertLaporan) {
    logger("[GIL] Gagal saat melakukan insert laporan");
    return res.status(500).json(resError("Gagal saat melakukan insert laporan"));
  }
  if (isUploading) {
    return res.status(200).json(resSuccess("Ada video yang sedang di proses upload"));
  }
  try {
