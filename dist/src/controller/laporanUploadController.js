...
const actionCountAntrian = async (req, res) => {
  ...
  if (!count) return res.status(500).json(resError("Gagal saat get count antrian"));
  return res.status(200).json(resSuccess("", {
    count: count.count
  }));
};
const actionListLaporan = async (req, res) => {
  ...
  if (!laporan) return res.status(500).json(resError("Gagal saat mengambil list data laporan"));
