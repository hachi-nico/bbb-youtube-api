...
const {
  getSecret,
  markExpSecret
} = require("../model/google_auth_secret");
const getAuthWithCallback = async (req, res) => {
  ...
  if (!secretFile) return res.status(500).json(resError("Ada parameter wajib yang kosong"));
  try {
...
const getAuthUrl = (req, res) => {
  ...
  try {
    fs.readFile(secretFile, (e, content) => {
      ...
      const oauth2Client = new OAuth2(secret.web.client_id, secret.web.client_secret, secret.web.redirect_uris[0]);
...
const getChannel = ({
  ...
  service.channels.list({
    ...
    if (err) return res.json(resError("gagal get channel", err.response.data));
...
const listVideoChannel = ({
  ...
  const handleDataFilter = data => {
    ...
    if (search) filteredData = filteredData.filter(item => item.judul.includes(search));
    return filteredData;
  };
  ...
  service.playlistItems.list({
    ...
    const buildData = response.data?.items?.length > 0 ? response.data.items.map(item => ({
    ...
    if (nextPageToken) {
      listVideoChannel({
        auth,
        res,
        nextPageTokenReq: nextPageToken
      });
    } else {
      const resData = structuredData[0];
      structuredData = [];
      const writeFile = fs.createWriteStream(cachePath);
      writeFile.write(JSON.stringify({
        count,
        data: resData
      }));
      writeFile.on("error", () => res.status(500).json(resError("Gagal saat write file json")));
      writeFile.on("finish", () => {
...
const youtubeUpload = async ({
  ...
  youtube.videos.insert({
    ...
    if (err) {
      const isQuotaExceed = err?.response?.data?.error?.errors[0].reason == "quotaExceeded" ? true : false;
      if (isQuotaExceed) {
        ...
        if (!updated) {
          ...
          return res.status(500).json({
            message: "Gagal saat mengubah status secret",
            err
          });
        }
    ...
    if (!updated) {
      ...
      return res.status(500).json(resError("Gagal saat update status menjadi berhasil"));
    }
    if (req?.app?.get("clientSub")) {
      sendNotification(req.app.get("clientSub"), "Video dengan judul " + additionalData.judul + " telah berhasil di upload ke youtube");
    }
    if (isNextAvailable) {
...
const updateVideo = ({
  ...
  if (!id) return res.status(500).json(resError("Ada parameter wajib yang kosong"));
  youtube.videos.update({
    ...
    console.log(err);
    if (err) return res.status(500).json(resError("Gagal saat update video", {
      err
    }));
    return res.status(200).json(resSuccess(""));
  });
