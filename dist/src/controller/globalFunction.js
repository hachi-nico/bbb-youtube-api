...
const logger = val => {
  ...
  try {
    fs.appendFile(cwd + "/.log/" + dayjs().format("MMM-YYYY").toString() + ".log", logValue, err => {
      if (err) console.log(dayjs().format(insertDateTimeFormat) + " Gagal Saat tulis log");
    });
