...
const actionUserList = async (req, res) => {
  ...
  if (!users) return res.status(500).json(resError("Gagal saat mengambil list data User"));
...
const actionAddUser = async (req, res) => {
  ...
  if (!username || !password || !tipe) return res.status(500).json(resError("Ada parameter wajib yang kosong"));
  if (tipe == 1) return res.status(500).json(resError("User dengan role Superuser tidak diperbolehkan untuk ditambah lagi"));
  try {
    const passwordHash = await bcrypt.hash(password, 10);
    const userCreated = await createUser(username, passwordHash, tipe, nama, tgl);
    if (!userCreated) return res.status(500).json(resError("Gagal saat membuat User"));
    return res.json(resSuccess(""));
  } catch (e) {
...
const actionUpdateUser = async (req, res) => {
  ...
  if (!userId) return res.status(500).json(resError("Ada parameter wajib yang kosong"));
  try {
    const userUpdated = await updateUser(username, tipe, nama, userId);
    if (!userUpdated) return res.status(500).json(resError("Gagal saat memperbarui User"));
    return res.json(resSuccess(""));
  } catch (e) {
...
const actionDeleteUser = async (req, res) => {
  ...
  try {
    ...
    if (!userDeleted) return res.status(500).json(resError("Gagal saat menghapus User"));
    return res.json(resSuccess(""));
  } catch (e) {
